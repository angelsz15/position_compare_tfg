...
void convert_tf_pose(const geometry_msgs::TransformStamped& transform) {
    tf::Quaternion quaternion;
    quaternion.setValue(transform.transform.rotation.x, transform.transform.rotation.y, transform.transform.rotation.z, transform.transform.rotation.w);
    tf::Matrix3x3 rpy(quaternion);
    double roll, pitch, yaw;
    rpy.getRPY(roll, pitch, yaw);

    ground_truth.pose.position.x = transform.transform.translation.x;
    ground_truth.pose.position.y = transform.transform.translation.y;
    ground_truth.pose.position.z = transform.transform.translation.z;
    ground_truth.pose.orientation = transform.transform.rotation;
...
void printFINALValue() {
    ...
    ROS_INFO("Número de datos medidos: %d", cont);
}
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber pose1_sub = nh.subscribe("/vicon/rover/rover", 10, convert_tf_pose);
